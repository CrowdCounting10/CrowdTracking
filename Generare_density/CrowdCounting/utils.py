...
def gen_x_y(img_paths, train_val_test='train', augmentation_methods=['ori']):
    ...
    for i in img_paths:
        ...
        if 'niente' in augmentation_methods:
            x.append(x_)
            y.append(y_)
...
def eval_loss(model, x, y, quality=False):
    ...
    if quality:
        ...
        for i in range(len(preds)):
            ...
            psnr = peak_signal_noise_ratio(preds[i], DM[i], data_range=data_range)
            ssim = structural_similarity(preds[i], DM[i], data_range=data_range)
            PSNR.append(psnr)
            SSIM.append(ssim)
    ...
    for i in sorted([os.path.join(path_file_root_curr, p) for p in paths]):
        with open(i, 'r') as fin:
            ...
            for l in fin.readlines():
                row= [l.replace(" ","").replace("[","").replace("]","").replace("'","").split(',')]
            i = 0
            while(i<len(row[0])):
                #print(str(row[0][i]))
                temp.append(row[0][i].replace('data/S','content/S'))
                i+=1
...
def ssim_loss(y_true, y_pred, c1=0.01 ** 2, c2=0.03 ** 2):
    ...
    weights_initial = tf.keras.backend.cast(weights_initial, tf.float32)
    ...
    sigma_F_squared = tf.nn.conv2d(tf.multiply(y_pred, y_pred), weights_initial, [1, 1, 1, 1],
                                   padding='SAME') - mu_F_squared
    sigma_Y_squared = tf.nn.conv2d(tf.multiply(y_true, y_true), weights_initial, [1, 1, 1, 1],
                                   padding='SAME') - mu_Y_squared
    sigma_F_Y = tf.nn.conv2d(tf.multiply(y_pred, y_true), weights_initial, [1, 1, 1, 1], padding='SAME') - mu_F_mu_Y

    ssim = ((2 * mu_F_mu_Y + c1) * (2 * sigma_F_Y + c2)) / (
            (mu_F_squared + mu_Y_squared + c1) * (sigma_F_squared + sigma_Y_squared + c2))

    return 1 - tf.reduce_mean(ssim, keepdims=[1, 2, 3])
...
def local_sum_loss(y_true, y_pred, alpha=0.5, grid_pooling=3):
    ...
    l1 = tf.keras.backend.square(tf.keras.backend.mean(tf.keras.backend.abs(y_true - y_pred)))
    l2 = tf.keras.backend.mean(tf.keras.backend.square(y_true - y_pred)) * 1000
